...
import {
  registerOrChangePasswordApi,
  sendCaptchaApi,
  checkPhoneExistenceApi
} from '@/api/login'
...
const captchaTimer = ref(null) // 验证码定时器
...
const handleRegister = async () => {
  ...
  await formRef.value.validate(async valid => {
    ...
    if (valid) {
      ...
      try {
        // 检查手机号是否已存在
        const { exist } = await checkPhoneExistenceApi({ phone: form.phone })
        if (exist === 1) {
          console.log('手机号已注册')
        ...
        // 调用注册接口
        const res = await registerOrChangePasswordApi({
          phone: form.phone,
